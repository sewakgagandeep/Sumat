...
export class OpenRouterProvider implements LLMProvider {
    ...
    constructor() {
        ...
        this.model = providerConfig?.model || 'anthropic/claude-sonnet-4-20250514';
        ...
        if (providerConfig?.apiKey) {
            this.client = new OpenAI({
                ...
                defaultHeaders: {
                    'HTTP-Referer': 'https://github.com/sumat',
                    'X-Title': 'Sumat AI Agent',
                },
            });
        }
    }
    ...
    async *chat(messages: Message[], options: ChatOptions): AsyncGenerator<StreamChunk> {
        if (!this.client) {
            yield { type: 'error', error: 'OpenRouter provider not configured' };
            return;
        }
        ...
        try {
            ...
            logger.error('OpenRouter chat error', { error: err.message });
