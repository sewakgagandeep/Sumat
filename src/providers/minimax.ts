...
export class MiniMaxProvider implements LLMProvider {
    ...
    constructor() {
        ...
        this.model = providerConfig?.model || 'MiniMax-Text-01';

        if (providerConfig?.apiKey) {
            const groupId = providerConfig.groupId || '';
            this.client = new OpenAI({
                apiKey: providerConfig.apiKey,
                baseURL: providerConfig.baseUrl || `https://api.minimax.chat/v1`,
            });
        }
    }
    ...
    async *chat(messages: Message[], options: ChatOptions): AsyncGenerator<StreamChunk> {
        if (!this.client) {
            yield { type: 'error', error: 'MiniMax provider not configured' };
            return;
        }
